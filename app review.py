...
def fetch_play_store_reviews_and_name(_app_id, lang='pt', country='br', count=MAX_REVIEWS_TO_PROCESS):
    ...
    except Exception: pass # st.warning já informa se der erro
    ...
    try:
        ...
        for r in app_reviews_data:
            if r['content']: review_texts.append(r['content']); review_scores.append(r['score'])
        if not review_texts: st.info(f"Nenhum review para: {app_name} ({_app_id}).")
        return review_texts, review_scores, app_name
    except Exception as e: st.error(f"Erro ao buscar reviews para {app_name}: {e}"); return [],[],app_name

# --- Funções de Análise com Gemini ---
# Adicionada INSTRUÇÃO IMPORTANTE SOBRE NOMES a todos os prompts de SÍNTESE e INSIGHTS.
...
def analyze_single_app_reviews(gemini_model_instance, reviews_text, app_name=""):
    ...
    p = f"""Analise reviews de '{app_name}'. {len(reviews_text.splitlines())} reviews. JSON: {{ "app_name":"{app_name}", "sentiment_summary":{{...}}, "top_topics":[...] }}
sentiment_summary: % Pos,Neu,Neg,SemSent. Soma 100%. top_topics: 5-7 temas (nome, menções P,N,Ng).
REVIEWS: {reviews_text}"""
    try:
        r = gemini_model_instance.generate_content(p).text.strip(); r = r.replace("```json", "").replace("```", "").strip()
        ...
        ts = sum(s.values()); s = {k:(v/ts*100 if ts>0 else 0) for k,v in s.items()} if abs(ts-100.0)>0.1 and ts!=0 else s
...
def extract_feature_details_from_reviews(gemini_model_instance, app_name, review_texts_str):
    ...
    p = f"""Reviews de '{app_name}'. JSON: 'elogiadas','problematicas','desejadas_ausentes'. Cada: 'funcionalidade','descricao_usuario'. Max 3-5/cat. REVIEWS: {review_texts_str}"""
    try:
        r = gemini_model_instance.generate_content(p).text.strip(); r = r.replace("```json", "").replace("```", "").strip()
        return json.loads(r)
...
def extract_pain_delight_points_from_reviews(gemini_model_instance, app_name, review_texts_str):
    ...
    p = f"""Reviews de '{app_name}'. JSON: 'pontos_dor'(3-5 frustrações),'fatores_encantamento'(3-5 satisfações). Cada: 'tipo','descricao','ex_review'. REVIEWS: {review_texts_str}"""
    try:
        r = gemini_model_instance.generate_content(p).text.strip(); r = r.replace("```json", "").replace("```", "").strip()
        return json.loads(r)
...
def generate_swot_analysis(gemini_model_instance, my_app_name, my_app_strengths, my_app_weaknesses, market_opportunities, market_threats):
    s,w,o,t = ["- "+"\n- ".join(l) if l else "N/A." for l in [my_app_strengths,my_app_weaknesses,market_opportunities,market_threats]]
...
def synthesize_audience_profile(gemini_model_instance, all_apps_collected_data_str, my_app_name_context): # my_app_name_context para dar contexto, mas o perfil é da categoria
...
def plot_comparative_sentiment_chart(all_apps_sentiment_data):
    ...
    df_filtered=df[[c for c in df.columns if df[c].sum()>0 and c in colors]]
...
with st.sidebar:
    ...
    my_app_url_st = st.text_input("URL Seu App (Foco):", key="my_app_url") # Removido _st para consistência
    competitor_urls_st = [st.text_input(f"URL Concorrente {i+1}:",key=f"comp{i}_url") for i in range(3)]
    competitor_urls_st = [url for url in competitor_urls_st if url] # Filtra vazios
    analyze_button = st.button("🔍 Analisar Aplicativos", type="primary", use_container_width=True)

# Inicialização do Estado da Sessão (mais concisa)
default_session_state = {
    ...
    'audience_profile_report': "", 'my_app_name_for_synthesis': "" # Renomeado para clareza
}
...
if analyze_button:
    if not my_app_url_st: st.sidebar.warning("Insira a URL do seu app.")
        ...
        for k, v in default_session_state.items(): st.session_state[k] = v # Reset
        ...
        with st.spinner("Análise em progresso... pode levar minutos."):
            ...
            for i, app_info in enumerate(urls_to_process):
                ...
                app_data = {"display_name": f"{app_info['role']}: {app_id if app_id else 'Inválido'}", "review_texts_str":"", "review_scores":[], "sentiment_topic_analysis":{}, "feature_details":{}, "pain_delight_points":{}}
                if app_id:
                    step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) Coleta: {app_info['role']}")
                    texts, scores, name = fetch_play_store_reviews_and_name(app_id)
                    d_name = f"{app_info['role']}: {name}" if app_info['role']!=name else name
                    app_data.update({"display_name": d_name, "review_scores": scores})
                    if app_info['role']=="Meu App": st.session_state.my_app_name_for_synthesis = d_name
                    if texts:
                        r_str = "\n".join(texts); app_data["review_texts_str"]=r_str
                        step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) Sent/Temas: {d_name}")
                        app_data["sentiment_topic_analysis"]=analyze_single_app_reviews(model,r_str,d_name)
                        step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) Features: {d_name}")
                        app_data["feature_details"]=extract_feature_details_from_reviews(model,d_name,r_str)
                        step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) Dor/Enc: {d_name}")
                        app_data["pain_delight_points"]=extract_pain_delight_points_from_reviews(model,d_name,r_str)
                    else: # Sem reviews
                        app_data["sentiment_topic_analysis"]={"app_name":d_name,"sentiment_summary":{"no_reviews":100.0},"top_topics":[]}
                else: st.warning(f"URL inválida para {app_info['role']}. Pulando.")
                processed_data.append(app_data)
            st.session_state.all_apps_processed_data = processed_data

            if processed_data:
                my_app_name = st.session_state.my_app_name_for_synthesis
                if not my_app_name and processed_data: my_app_name = processed_data[0]["display_name"]; st.session_state.my_app_name_for_synthesis = my_app_name
                
                # Sínteses Globais
                step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) GAPs Features...")
                feat_list=[{"app_name":d["display_name"],**d["feature_details"]} for d in processed_data if d.get("feature_details")]
                ...
                pain_list=[{"app_name":d["display_name"],**d["pain_delight_points"]} for d in processed_data if d.get("pain_delight_points")]
                ...
                sent_topic_list=[d["sentiment_topic_analysis"] for d in processed_data if d.get("sentiment_topic_analysis") and "no_reviews" not in d["sentiment_topic_analysis"].get("sentiment_summary",{})]
                if sent_topic_list and my_app_name:
                    ...
                    step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) SWOT {my_app_name}...")
                    q_data=st.session_state.overall_qualitative_data
                    my_app_q_an=next((item for item in q_data.get("analises_individuais",[]) if item.get("app_name")==my_app_name),None)
                    if my_app_q_an: st.session_state.swot_report = generate_swot_analysis(model,my_app_name,my_app_q_an.get("pontos_fortes",[]),my_app_q_an.get("pontos_fracos",[]),q_data.get("oportunidades_mercado",[]),q_data.get("ameacas_desafios_mercado",[]))
                
                step+=1; prog_bar.progress(min(1.0,step/total_steps)); prog_text.info(f"({step}/{total_steps}) Perfil Público...")
                aud_prompt_data=[{"app_name":d["display_name"],"sentiment":d["sentiment_topic_analysis"].get("sentiment_summary"),"top_topics":d["sentiment_topic_analysis"].get("top_topics"),"requested_features":[f.get('funcionalidade') for f in d["feature_details"].get("desejadas_ausentes",[])],"pain_points":[p.get('descricao') for p in d["pain_delight_points"].get("pontos_dor",[])],"delight_factors":[dl.get('descricao') for dl in d["pain_delight_points"].get("fatores_encantamento",[])]} for d in processed_data]
                if aud_prompt_data and my_app_name: st.session_state.audience_profile_report = synthesize_audience_profile(model,json.dumps(aud_prompt_data,ensure_ascii=False,indent=2),my_app_name)
...
if st.session_state.analysis_complete and st.session_state.all_apps_processed_data:
    ...
    tab_names = ["📊 Sentimento Geral", "📱 Apps Individuais", "🧩 GAPs de Features", "❤️ Dor vs. Encantamento", "♟️ Matriz SWOT", "👤 Perfil do Público", "💡 Qualitativa Estratégica"]
    ...
    with tabs[1]: # Apps Individuais
        ...
        for app_data in st.session_state.all_apps_processed_data:
            d_name = app_data["display_name"]
            is_my_app = st.session_state.my_app_name_for_synthesis == d_name
            with st.expander(f"Detalhes: {d_name}", expanded=is_my_app):
                st.markdown(f"**Distribuição de Notas**")
                if app_data["review_scores"]:
                    r_dist_str,_ = plot_ratings_distribution_chart(d_name,app_data["review_scores"])
                    if r_dist_str and "Nenhuma nota" not in r_dist_str and model: st.markdown(f"**Análise IA:**\n{analyze_ratings_insights(model,d_name,r_dist_str)}")
                else: st.caption(f"Sem notas.")
                st.markdown("---")
                st.markdown(f"**Principais Temas**")
                s_t_an = app_data["sentiment_topic_analysis"]
                if s_t_an and "no_reviews" not in s_t_an.get("sentiment_summary",{}) and "error" not in s_t_an.get("sentiment_summary",{}) and s_t_an.get("top_topics"):
                    ...
                    with st.popover("Dados JSON dos temas"): st.json(s_t_an.get("top_topics",[]))
                else: st.caption(f"Sem temas/reviews.")
                st.markdown("---")
                st.markdown(f"**Detalhes de Funcionalidades (Extraído)**")
                if app_data.get("feature_details") and any(app_data["feature_details"].values()): st.json(app_data["feature_details"])
                else: st.caption(f"Não extraído.")
                st.markdown("---")
                st.markdown(f"**Pontos de Dor e Encantamento (Extraído)**")
                if app_data.get("pain_delight_points") and any(app_data["pain_delight_points"].values()): st.json(app_data["pain_delight_points"])
                else: st.caption(f"Não extraído.")
    ...
    with tabs[4]: st.subheader(f"{tab_names[4]} ({st.session_state.my_app_name_for_synthesis})"); st.markdown(st.session_state.swot_report if st.session_state.swot_report else "Não gerado.")
    ...
    with tabs[6]:
        ...
        if q_data and q_data.get("analises_individuais"):
            ...
            for app_an in q_data.get("analises_individuais",[]):
                ...
                st.markdown(f"* Fortes: {'; '.join(app_an.get('pontos_fortes',['N/A']))}")
                st.markdown(f"* Fracos: {'; '.join(app_an.get('pontos_fracos',['N/A']))}"); st.markdown("---")
            ...
            my_app_n_s = st.session_state.my_app_name_for_synthesis if st.session_state.my_app_name_for_synthesis else "Seu App"
